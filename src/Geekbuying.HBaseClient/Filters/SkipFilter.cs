...
namespace Geekbuying.HBaseClient.Filters
{
    ...
    /// <remarks>
    /// A wrapper filter that filters an entire row if any of the Cell checks do not pass.
    /// For example, if all columns in a row represent weights of different things, with the values being the actual weights,
    /// and we want to filter
    /// out the entire row if any of its weights are zero. In this case, we want to prevent rows from being emitted if a single
    /// key is filtered.
