...
{
    /// <summary>
    /// Implementation of <see cref="Filter" /> that represents an ordered List of Filters which will be evaluated with a
    /// specified boolean operator
    ...
    public class FilterList : Filter
    {
        ...
        public FilterList(Operator op, IEnumerable<Filter> rowFilters)
        {
            ...
            {
                if (ReferenceEquals(f, null)) throw new ArgumentContainsNullException("rowFilters", null, null);
        ...
        /// The filters.
        /// </value>
        public IEnumerable<Filter> Filters => _rowFilters;
        ...
        {
            if (_rowFilters.Count == 0) return string.Empty;

            var working = new StringBuilder();
            foreach (var f in _rowFilters) working.AppendFormat(@"{0},", f.ToEncodedString());
