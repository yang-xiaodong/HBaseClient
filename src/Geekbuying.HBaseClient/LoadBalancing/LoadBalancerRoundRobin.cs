...
namespace Geekbuying.HBaseClient.LoadBalancing
{
    /// <summary>
    ...
    public class LoadBalancerRoundRobin : ILoadBalancer
    {
        ...
        {
            _workerHostNamePrefix = ReadFromConfig(Constants.WorkerHostNamePrefixConfigKey, string.Copy,
                Constants.WorkerHostNamePrefixDefault);
            _workerRestEndpointPort = ReadFromConfig(Constants.WorkerRestEndpointPortConfigKey, int.Parse,
                Constants.WorkerRestEndpointPortDefault);

            var refreshIntervalFromConfig = ReadFromConfig(Constants.RefreshIntervalInMillisecondsConfigKey, int.Parse,
                Constants.RefreshIntervalInMillisecondsDefault);
        ...
        internal static T ReadFromConfig<T>(string configKey, Func<string, T> parseConfigValue, T defaultValue)
        {
            ...
            {
                Trace.TraceWarning("Failed to configure parameter with key {0}, failling back on default value {1}",
