...
namespace Geekbuying.HBaseClient.Internal
{
    internal static class ArgumentGuardExtensions
    {
        ...
        {
            if (ReferenceEquals(value, null)) throw new ArgumentNullException(argumentName ?? string.Empty);
        }

        internal static void ArgumentNotNullNorContainsNull<T>([ValidatedNotNull] this IEnumerable<T> value,
        ...
        {
            value.ArgumentNotNull(paramName);
            if (!value.Cast<object>().Any()) throw new ArgumentEmptyException(paramName ?? string.Empty, null, null);
        }
