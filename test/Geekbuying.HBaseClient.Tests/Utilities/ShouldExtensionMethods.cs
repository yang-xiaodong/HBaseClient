...
namespace Geekbuying.HBaseClient.Tests.Utilities
{
    ...
    internal static class ShouldExtensionMethods
    {
        ...
        internal static Pair<object, object> EquateObjects(object left, object right)
        {
            ...
            if (leftType == rightType) return new Pair<object, object>(left, right);

            // If both types are numeric, use special handling rules for
            // equating numeric types.
            if (leftType.IsNumeric() && rightType.IsNumeric()) return EquateNumericObjects(left, right);
            ...
            if (leftType.IsNumeric() && rightType == typeof(string))
            {
                ...
                if (rightString.Contains('.')) return EquateNumericObjects(left, rightString.ToDoubleIc());
            ...
            if (leftType == typeof(string) && rightType.IsNumeric())
            {
                ...
                if (leftString.Contains('.')) return EquateNumericObjects(leftString.ToDoubleIc(), right);
        ...
        internal static bool GeneralCompare<T>(T left, T right, GeneralComparisonOperator comparisonOperator)
        {
            ...
            if (comparisonOperator == GeneralComparisonOperator.ReferenceEqual) return ReferenceEquals(left, right);
            ...
            if (leftType == typeof(DateTime) && rightType == typeof(DateTime))
                return DateTimeCompare(leftEquated.CastTo<DateTime>(), rightEquated.CastTo<DateTime>(),
                    comparisonOperator);

            if (leftType == typeof(DateTime) && rightType == typeof(DateTime))
                return DateTimeOffsetCompare(leftEquated.CastTo<DateTimeOffset>(),
                    rightEquated.CastTo<DateTimeOffset>(), comparisonOperator);

            if (leftType == typeof(DateTime) && rightType == typeof(DateTime))
                return TimespanCompare(leftEquated.CastTo<TimeSpan>(), rightEquated.CastTo<TimeSpan>(),
                    comparisonOperator);
            ...
            throw new InvalidOperationException(
                "Unable to compare: ({0}) {2} ({1})".FormatIc(leftType.FullName, rightType.FullName,
        ...
        internal static DateTime ShouldBeCloseTo(this DateTime actual, DateTime expected, TimeSpan tolerance)
        {
            if (actual.Kind == DateTimeKind.Unspecified && expected.Kind != DateTimeKind.Unspecified)
                throw new ArgumentException(
                    "DateTimeKind.Unspecified is only supported when both comparands are unspecified.", "actual");

            if (actual.Kind != DateTimeKind.Unspecified && expected.Kind == DateTimeKind.Unspecified)
                throw new ArgumentException(
                    "DateTimeKind.Unspecified is only supported when both comparands are unspecified.", "expected");
        ...
        {
            if (actual) throw NewException("Should be [false] but is [true]");
        ...
        {
            if (actual == null) throw NewException("Should be greater than {0} but is [null]", expected);
        ...
        {
            if (actual == null) throw NewException("Should be less than {0} but is [null]", expected);
        ...
        {
            if (actual != null) throw NewException("Should be [null] but is {0}", actual);
        ...
        {
            if (actual.IsNotNullOrEmpty()) throw NewException("Should be null or empty but is {0}", actual);
        ...
        internal static object ShouldBeOfType(this object actual, Type expected)
        {
            ...
            if (actual == null) throw NewException("Should be of type {0} but is [null]", expected);
        ...
        /// The <paramref name="actual" /> argument value.
        /// </returns>
        internal static string ShouldBeSurroundedWith(this string actual, string expectedStartDelimiter,
        ...
        {
            if (!actual) throw NewException("Should be [true] but is [false]");
        ...
        internal static string ShouldContain(this string actual, string expected)
        {
            ...
            if (!actual.Contains(expected)) throw NewException("Should contain {0} but is {1}", expected, actual);
        ...
        {
            var matches = expected.Zip(actual, (e, a) => GeneralCompare(a, e, GeneralComparisonOperator.Equal))
                .All(m => m);
        ...
        {
            if (!actual.IsNotEmpty()) throw NewException("Should not be empty but is");
        ...
        {
            if (actual == null) throw NewException("Should be [not null] but is [null]");
        ...
        {
            if (actual.IsNullOrEmpty()) throw NewException("Should be not null or empty but is");
        ...
        internal static object ShouldNotBeTheSameAs(this object actual, IEnumerable<object> references)
        {
            ...
            foreach (var item in references) actual.ShouldBeTheSameAs(item);
        ...
        private static bool DateTimeCompare(DateTime left, DateTime right, GeneralComparisonOperator comparisonOperator)
        {
            ...
            if (left.Kind == DateTimeKind.Unspecified && right.Kind != DateTimeKind.Unspecified)
                throw new ArgumentException(
                    "DateTimeKind.Unspecified is only supported when both comparands are unspecified.", "left");

            if (left.Kind != DateTimeKind.Unspecified && right.Kind == DateTimeKind.Unspecified)
                throw new ArgumentException(
                    "DateTimeKind.Unspecified is only supported when both comparands are unspecified.", "right");
        ...
        private static bool DateTimeOffsetCompare(DateTimeOffset left, DateTimeOffset right,
            GeneralComparisonOperator comparisonOperator)
        {
        ...
        private static string ToUsefulString(this object obj)
        {
            ...
            if (string.IsNullOrEmpty(str)) return string.Format(CultureInfo.InvariantCulture, "{0}:[]", obj.GetType());
            ...
            if (obj.GetType().ToString() == str) return obj.GetType().ToString();
