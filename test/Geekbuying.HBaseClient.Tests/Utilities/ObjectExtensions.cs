...
namespace Geekbuying.HBaseClient.Tests.Utilities
{
    ...
    internal static class ObjectExtensions
    {
        ...
        private static readonly Type[] SignedTypes =
            TestHelp.Array(typeof(sbyte), typeof(short), typeof(int), typeof(long));

        private static readonly Type[] UnsignedTypes =
            TestHelp.Array(typeof(bool), typeof(byte), typeof(ushort), typeof(uint), typeof(ulong));
        ...
        /// </summary>
        internal static IEnumerable<Type> KnownTypes { get; } = FindKnownTypes();
        ...
        /// </returns>
        private static object InternalDynaCreate(Type concreteType, Type nullType, BindingFlags bindings,
            object[] parameters)
        {
            // Handel CLRs "implicit constructor" for value types.
            if (concreteType.IsValueType && (Internal.ObjectExtensions.IsNull(parameters) || parameters.Length == 0))
                return Activator.CreateInstance(concreteType);

            var bestMatch = LocateBestMatch<ConstructorInfo>(concreteType, null, nullType, bindings, parameters, null);

            if (bestMatch != null) return InternalDynaInvoke(bestMatch, parameters);
        ...
        internal static IEnumerable<Type> FindKnownTypes()
        {
            ...
            foreach (var reference in asm.GetReferencedAssemblies()) Assembly.Load(reference.FullName);
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                ...
                catch (ReflectionTypeLoadException rtle)
                {
                    ...
                    {
                        if (Internal.ObjectExtensions.IsNull(type)) continue;
        ...
        internal static object Get(this IDictionary<string, object> expando, string name)
        {
            ...
            if (asDictionary.ContainsKey(name)) return asDictionary[name];
        ...
        /// The return value of the method invoked.
        /// </returns>
        internal static object Invoke(this object instance, string methodName, Type nullType,
            params object[] parameters)
        {
            if (instance == null) throw new ArgumentNullException("instance");

            const BindingFlags Bindings = BindingFlags.InvokeMethod | BindingFlags.Instance | BindingFlags.Public |
                                          BindingFlags.NonPublic;
            var type = instance.GetType();
            var bestMatch = LocateBestMatch<MethodInfo>(type, methodName, nullType, Bindings, parameters, null);

            if (bestMatch != null) return InternalDynaInvoke(bestMatch, parameters, instance);

            throw new MissingMethodException(string.Format(CultureInfo.InvariantCulture,
                "The method {0} does not exist", methodName));
        }
        ...
        /// The return value of the method invoked.
        /// </returns>
        internal static object Invoke(this object instance, string methodName, Type[] typeParameters,
            params object[] parameters)
        {
        ...
        /// The return value of the method invoked.
        /// </returns>
        internal static object Invoke(this object instance, string methodName, Type[] typeParameters, Type nullType,
            params object[] parameters)
        {
            if (instance == null) throw new ArgumentNullException("instance");
            const BindingFlags Bindings = BindingFlags.InvokeMethod | BindingFlags.Instance | BindingFlags.Public |
        ...
        {
            const BindingFlags Bindings = BindingFlags.InvokeMethod | BindingFlags.Static | BindingFlags.Public |
                                          BindingFlags.NonPublic;
            var bestMatch = LocateBestMatch<MethodInfo>(type, methodName, nullType, Bindings, parameters, null);

            if (bestMatch != null) return InternalDynaInvoke(bestMatch, parameters, null);
        ...
        /// The return value of the method invoked.
        /// </returns>
        internal static object Invoke(this Type type, string methodName, Type[] typeParameters,
            params object[] parameters)
        {
        ...
        /// The return value of the method invoked.
        /// </returns>
        internal static object Invoke(this Type type, string methodName, Type nullType, Type[] typeParameters,
            params object[] parameters)
        {
            const BindingFlags Bindings = BindingFlags.InvokeMethod | BindingFlags.Static | BindingFlags.Public |
                                          BindingFlags.NonPublic;
            ...
            var bestMatch =
                LocateBestMatch<MethodInfo>(type, methodName, nullType, Bindings, parameters, typeParameters);
        ...
        internal static bool IsParamsParameter(this ParameterInfo parameterInfo)
        {
            ...
            return parameterInfo.GetCustomAttributes(true).Where(a => a.GetType() == typeof(ParamArrayAttribute))
                       .Count() > 0;
        }
        ...
        internal static string SubstringAfter(this string inputValue, string searchValue)
        {
            ...
            if (loc >= 0 && loc < inputValue.Length + 1) return inputValue.Substring(loc + 1);
        ...
        internal static string SubstringBefore(this string inputValue, string searchValue)
        {
            ...
            var loc = inputValue.IndexOfOi(searchValue);
            if (loc >= 0) return inputValue.Substring(0, loc);
        ...
        [SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity",
            Justification =
                "NEIN: This should be refactored to reduce the complexity, letting it go for now. [tistocks]")]
        private static int CompareTypeAssignments(ParameterInfo[] parameters, object[] values, Type nullType,
            Type[] typeParameters)
        {
            TestHelp.DoNothing(typeParameters);

            var paramCount = parameters != null ? parameters.Count() : 0;
            var objCount = values != null ? values.Count() : 0;
            if (paramCount == 0 && objCount == 0) return int.MaxValue;
            ...
            for (var i = 0; i < max; i++)
                if (i < paramCount && i < objCount)
                {
                    ...
                    if (info.GetCustomAttributes(true)
                            .Where(a => a.GetType().IsAssignableFrom(typeof(ParamArrayAttribute))).Count() >
                        0) paramsParameter = info;
                    ...
                    else if (infoType.IsArray && objType.IsArray)
                    {
                        ...
                        if (infoElementType.ContainsGenericParameters)
                        {
                            ...
                            if (genericParameterConstraints.Count() == 0 ||
                                genericParameterConstraints.Any(constraint =>
                                    constraint.IsAssignableFrom(objElementType)))
                                retval += 3;
                        }
                    }
                    else if (infoType.IsArray && Internal.ObjectExtensions.IsNotNull(paramsParameter))
                    {
                        var infoElementType = infoType.GetElementType();
                    ...
                    else
                    {
                        ...
                        else if (infoType.ContainsGenericParameters)
                        {
                            ...
                            if (infoType.Name == objType.Name) retval += 3;
                        }
                        else if (typeof(IEnumerable).IsAssignableFrom(infoType) &&
                ...
                else if (i >= paramCount && i < objCount && Internal.ObjectExtensions.IsNotNull(paramsParameter))
                {
        ...
        {
            if (Internal.ObjectExtensions.Is<ExpandoObject>(type) ||
                Internal.ObjectExtensions.Is<IDictionary<string, object>>(type))
                return Get(instance.As<IDictionary<string, object>>(), name);

            var staticOrInstance =
                Internal.ObjectExtensions.IsNull(instance) ? BindingFlags.Static : BindingFlags.Instance;
            var bindings = BindingFlags.Public | BindingFlags.NonPublic | staticOrInstance;
            var info = type.GetMember(name, bindings | BindingFlags.GetField | BindingFlags.GetProperty)
                .FirstOrDefault();

            if (info == null)
                throw new MissingMemberException(string.Format(CultureInfo.InvariantCulture,
                    "The member {0} does not exist", name));

            var infoType = info.GetType();

            if (Internal.ObjectExtensions.Is<PropertyInfo>(infoType)) return Get(instance, (PropertyInfo) info);

            if (Internal.ObjectExtensions.Is<FieldInfo>(infoType)) return Get(instance, (FieldInfo) info);
        ...
        private static object InternalDynaInvoke(MethodBase method, object[] parameters, object instance = null)
        {
            try
            {
                ...
                if (Internal.ObjectExtensions.IsNotNull(instance) && method.IsStatic)
        ...
        /// parameters.
        /// </returns>
        private static T LocateBestMatch<T>(Type type, string name, Type nullType, BindingFlags bindings,
            object[] parameters, Type[] typeParameters)
            where T : MethodBase
        {
            ...
            if (typeof(MethodInfo).IsAssignableFrom(typeof(T)))
            {
                ...
                extensions = sameNamedExtensionMethods
                    .Where(m => m.GetParameters()[0].ParameterType.IsAssignableFrom(type)).Cast<T>();
            ...
            if (Internal.ObjectExtensions.IsNull(bestMatch) && Internal.ObjectExtensions.IsNotNull(extensions) &&
                typeof(MethodInfo).IsAssignableFrom(typeof(T)))
                foreach (var info in extensions)
                {
                    // This is used to "score" the best constructor to call.
                    // This avoids unnecessary ambiguity errors when multiple members
                    // "could work".
                    var score = CompareTypeAssignments(info.GetParameters().Skip(1).ToArray(), parameters, nullType,
                        null);
                    if (score > bestScore)
                    {
                        bestScore = score;
        ...
        {
            if (Internal.ObjectExtensions.Is<IDictionary<string, object>>(type))
            {
            ...
            if (Internal.ObjectExtensions.Is<PropertyInfo>(infoType))
                Set(instance, (PropertyInfo) info, value);
            else if (Internal.ObjectExtensions.Is<FieldInfo>(infoType))
