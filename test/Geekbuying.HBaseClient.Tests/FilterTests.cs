...
using System.Linq;
using System.Text;
using Geekbuying.HBaseClient.Filters;
    ...
    public class FilterTests : DisposableContextSpecification
    {
        ...
        private IEnumerable<long> RetrieveTimestamps(ScannerInformation scanInfo, RequestOptions scanOptions)
        {
            ...
            while ((next = client.ScannerGetNextAsync(scanInfo, scanOptions).Result) != null)
                foreach (var row in next.rows)
                {
                    ...
                    foreach (var c in cells) rv.Add(c.timestamp);
                }
        ...
        private IEnumerable<FilterTestRecord> RetrieveResults(ScannerInformation scanInfo, RequestOptions scanOptions)
        {
            ...
            while ((next = client.ScannerGetNextAsync(scanInfo, scanOptions).Result) != null)
                foreach (var row in next.rows)
                {
                    var rowKey = _encoding.GetString(row.key);
        ...
        private void PopulateTable()
        {
            ...
            for (var lineNumber = 0; lineNumber < 10; ++lineNumber)
            {
                ...
                var rec = new FilterTestRecord(rowKey, lineNumber, Guid.NewGuid().ToString("N"),
                ...
                {
                    column = BuildCellColumn(ColumnFamilyName1, ColumnNameA),
                    data = _encoding.GetBytes(rec.A)
                };
                row.values.Add(paragraphColumnValue);

                var columnValueB = new Cell
                {
                    column = BuildCellColumn(ColumnFamilyName2, ColumnNameB),
                    data = Encoding.UTF8.GetBytes(rec.B)
                };
        ...
        {
            return _encoding.GetBytes(string.Format(CultureInfo.InvariantCulture, "{0}:{1}", columnFamilyName,
                columnName));
        }

        private string ExtractColumnName(byte[] cellColumn)
        {
            var qualifiedColumnName = _encoding.GetString(cellColumn);
        ...
        public void When_I_Scan_all_I_get_the_expected_results()
        {
            ...
            {
                if (scanInfo != null) client.DeleteScannerAsync(_tableName, scanInfo, scanOptions).Wait();
            }
        }
        ...
        {
            var expectedRecords =
                (from r in _allExpectedRecords select r.WithLineNumberValue(0).WithBValue(null)).ToList();

            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new ColumnRangeFilter(Encoding.UTF8.GetBytes(ColumnNameA), true,
                Encoding.UTF8.GetBytes(ColumnNameB), false);
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
            ScannerInformation scanInfo = null;
        ...
        [Fact]
        public void
            When_I_Scan_with_a_DependentColumnFilter_and_a_BinaryComparator_with_the_operator_equal_I_get_the_expected_results()
        {
        ...
        public void When_I_Scan_with_a_FamilyFilter_I_get_the_expected_results()
        {
            ...
            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new FamilyFilter(CompareFilter.CompareOp.Equal,
                new BinaryComparator(Encoding.UTF8.GetBytes(ColumnFamilyName1)));
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
            ScannerInformation scanInfo = null;
        ...
        public void When_I_Scan_with_a_FirstKeyOnlyFilter_I_get_the_expected_results()
        {
            ...
            var expectedRecords =
                (from r in _allExpectedRecords select new FilterTestRecord(r.RowKey, 0, string.Empty, string.Empty))
                .ToList();
        ...
        public void When_I_Scan_with_a_PrefixFilter_I_get_the_expected_results()
        {
            ...
            var expectedRecords = (from r in _allExpectedRecords
                ...
                where rawKey[0] == prefix[0] && rawKey[1] == prefix[1] && rawKey[2] == prefix[2] &&
                      rawKey[3] == prefix[3]
                select r).ToList();
        ...
        public void When_I_Scan_with_a_QualifierFilter_I_get_the_expected_results()
        {
            ...
            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new QualifierFilter(CompareFilter.CompareOp.Equal,
                new BinaryComparator(Encoding.UTF8.GetBytes(LineNumberColumnName)));
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
            ScannerInformation scanInfo = null;
        ...
        public void When_I_Scan_with_a_RowFilter_I_get_the_expected_results()
        {
            ...
            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new RowFilter(CompareFilter.CompareOp.Equal,
                new BinaryComparator(Encoding.UTF8.GetBytes(example.RowKey)));
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
            ScannerInformation scanInfo = null;
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueExcludeFilter_and_a_BinaryComparator_with_the_operator_equal_I_get_the_expected_results()
        {
            var bValue = (from r in _allExpectedRecords select r.B).First();

            // B column should not be returned, so set the value to null.
            var expectedRecords =
                (from r in _allExpectedRecords where r.B == bValue select r.WithBValue(null)).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_equal_I_get_the_expected_results()
        {
            var expectedRecords = (from r in _allExpectedRecords where r.LineNumber == 1 select r).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_greater_than_I_get_the_expected_results()
        {
            var expectedRecords = (from r in _allExpectedRecords where r.LineNumber > 1 select r).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_less_than_I_get_the_expected_results()
        {
            var expectedRecords = (from r in _allExpectedRecords where r.LineNumber < 1 select r).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_less_than_or_equal_I_get_the_expected_results(
            )
        {
            var expectedRecords = (from r in _allExpectedRecords where r.LineNumber <= 1 select r).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_no_op_I_get_the_expected_results()
        {
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryComparator_with_the_operator_not_equal_I_get_the_expected_results()
        {
            var expectedRecords = (from r in _allExpectedRecords where r.LineNumber != 1 select r).ToList();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_BinaryPrefixComparator_with_the_operator_equal_I_get_the_expected_results()
        {
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_NullComparator_with_the_operator_not_equal_I_get_the_expected_results()
        {
            var expectedRecords = new List<FilterTestRecord>();
        ...
        [Fact]
        public void
            When_I_Scan_with_a_SingleColumnValueFilter_and_a_SubstringComparator_with_the_operator_equal_I_get_the_expected_results()
        {
        ...
        public void When_I_Scan_with_a_SkipFilter_I_get_the_expected_results()
        {
            ...
            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new SkipFilter(new ValueFilter(CompareFilter.CompareOp.NotEqual,
                new BinaryComparator(BitConverter.GetBytes(0))));
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
            ScannerInformation scanInfo = null;
        ...
        public void When_I_Scan_with_a_TimestampsFilter_I_get_the_expected_results()
        {
            ...
            {
                if (scanAll != null) client.DeleteScannerAsync(_tableName, scanAll, scanOptions).Wait();
            }
        ...
        {
            var expectedRecords =
                (from r in _allExpectedRecords where r.LineNumber == 3 select r.WithAValue(null).WithBValue(null))
                .ToList();
        ...
        {
            var expectedRecords =
                (from r in _allExpectedRecords where r.LineNumber == 0 select r.WithBValue(null)).ToList();

            var client = new HBaseClient(_credentials);
            var scanner = new Scanner();
            var filter = new WhileMatchFilter(new ValueFilter(CompareFilter.CompareOp.NotEqual,
                new BinaryComparator(BitConverter.GetBytes(0))));
            scanner.filter = filter.ToEncodedString();
            var scanOptions = RequestOptions.GetDefaultOptions();
            scanOptions.AlternativeEndpoint = Constants.RestEndpointBaseZero;
