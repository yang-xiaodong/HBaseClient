...
namespace Geekbuying.HBaseClient.Tests
{
    public class CellOperationsTests : DisposableContextSpecification
    {
        ...
        private Cell GetCell(string key, string columnName, string value = null, long timestamp = 0)
        {
            var cell = new Cell
            {
                column = BuildCellColumn(ColumnFamilyName1, columnName),
                row = Encoding.UTF8.GetBytes(key)
            };
            if (value != null) cell.data = Encoding.UTF8.GetBytes(value);
            if (timestamp > 0) cell.timestamp = timestamp;
            return cell;
        }
        ...
        private CellSet.Row GetCellSet(string key, string columnName, string value, long timestamp)
        {
            ...
            if (value != null) c1.data = Encoding.UTF8.GetBytes(value);

            if (timestamp > 0) c1.timestamp = timestamp;
            row.values.Add(c1);
            return row;
        }
        ...
        public async Task When_I_CheckAndDeleteCells_With_ColumnFamily_Deletes_All_cells()
        {
            ...
            var rowToDelete = new CellSet.Row {key = Encoding.UTF8.GetBytes("3")};
        ...
        public async Task When_I_CheckAndDeleteCells_With_TimeStamp_I_cannot_add_with_lower_timestamp_than_hbaseserver()
        {
            ...
            try
            {
                ...
                throw new AssertFailedException(
                    "Expected to throw an exception as the row cannot be added with lower timestamp than servers timestamp");
            }
            catch (Exception ex)
            {
                if (ex is AggregateException)
